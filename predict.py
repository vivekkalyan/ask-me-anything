...
q_vocab, a_vocab = vocab.retrieve_vocab(config.vocab_questions_path, config.vocab_answers_path, config.questions_train_path, config.annotations_train_path)
...
transform = image_features.get_transform(config.image_size, config.scale_fraction)
...
def predict(img_path, qn): 
    ...
    topk_ans = [next(k for k, v in a_vocab.items() if v == idx) for idx in topk_idx]
...
if __name__ == '__main__':
    predict('./data/val2014/COCO_val2014_000000000328.jpg', 'how many people are there')
